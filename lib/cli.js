...
async function run() {
  ...
  program
    .command("pull")
    .description("Sync origin with all the new or modified components from an account")
    .argument("[account_id]", "accountId")
    .action((accountId) => {
      pull(accountId);
    });
...
function cloneAppSelectorCLI(callback, accountId) {
  ...
  if (accountId) {
    if (appFolders.includes(accountId)) {
      ...
      rl.question("This app already exists. Do you want to overwrite it? [Y/n] ", (answer) => {
