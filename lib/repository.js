...
const RpcManager = {
  ...
  getWidgets: async (accountId, widgetsPath) => {
    ...
    const widgets = chunks.map(async (chunk) => {
      ...
      const result = await RpcManager.send(
        RpcManager.createRequest(request)
      );
      
      const response = await result.json();
      
      const { [accountId]: { widget } } = RpcManager.parseResponse(response);
  
      return widget;
    })

    return await Promise.all(widgets).then((result) => result.reduce((acc, obj) => ({...acc, ...obj}), {}));
  },
  getAccountWidgetKeys: async (accountId, widgetsPath) => {
    ...
    const { [accountId]: { widget: keys } } = RpcManager.parseResponse(response);
...
async function pull(accountId) {
  ...
  if (!Object.keys(accountTree).length) {
    console.log("Cannot sync. You have to clone the project first using: bos-workspace clone " + accountId);
    return;
  }
...
async function remove(accountId) {
  ...
  if (!Object.keys(accountTree).length) {
    console.error("Cannot remove account. You don't have components from " + accountId);
    return;
  }
