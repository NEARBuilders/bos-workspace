...
function init_bos_config(appName, version = "") {
  const configPath = path.join("./apps", appName, "bos.config.json");

  if (!fs.existsSync(configPath)) {
    const config = JSON.stringify({ appAccount: appName, version });

    try {
      fs.writeFileSync(configPath, config);
      console.log("\nInitialized bos.config.json for " + appName + "at version " + version);
