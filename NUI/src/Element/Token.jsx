...
const direction = props.direction ?? "right"; // left or right

address = address == "" ? "near" : address;
const isNEAR =
  address.toLowerCase() === "near" || address.toLowerCase() === "wrap.near";
...
const Wrapper = styled.div`
  ...
  grid-column-gap: 8px;
  grid-template-columns: ${direction === "left" ? "32px 1fr" : "1fr 32px"};
  text-align: ${direction === "left" ? "left" : "right"};

  .icon {
    grid-row-start: 1;
    grid-row-end: 3;
    grid-column: ${direction === "left" ? 1 : 2};
  }
  .amount {
    font-size: 18px;
    font-weight: 500;
    line-height: 1.15;
    grid-column: ${direction === "left" ? 2 : 1};
  }
  .symbol {
    font-size: 12px;
    font-weight: 500;
